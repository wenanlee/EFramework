...
namespace EFramework.Unity.XNodeEditor
{
    ...
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, XNode.NodeGraph>
    {
        ...
        public virtual string GetNodeName(Type type)
        {
            //Check if type has the CreateNodeMenuAttribute
            XNode.Node.CreateNodeMenuAttribute attrib;
            if (NodeEditorUtilities.GetAttrib(type, out attrib)) // Return custom name
                return attrib.nodeName;
            else // Return generated name
                return NodeEditorUtilities.NodeDefaultName(type);
        }
        ...
        public virtual XNode.Node CreateNode(Type type, Vector2 position)
        {
            ...
            if (nodeName != null && nodeName.Trim() != "") node.name = nodeName;
