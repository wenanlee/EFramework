...
namespace CommandTerminal
{
    ...
    public class Terminal : MonoBehaviour
    {
        ...
        string url = @"http://47.110.142.168:8080/insert?name=Neo2&log=[{" + "\"level\":{},\"time\":\"{}\",\"info\":\"{}\"}]";
        void HandleUnityLog(string message, string stack_trace, LogType type) {
            ...
            {
                Debug.Log(url);
                //HttpRequestHelper.HttpGetRequestAsync(string.Format(url,"1","","test"));
                //HttpRequestHelper.HttpGetRequestAsync(@"http://47.110.142.168:8080/insert?name=Neo2&log=[{" + "\"level\":"+type.GetHashCode()+",\"time\":\"0\",\"info\":\""+message+"\"}]");
                //Debug.Log(string.Format(url, type.GetHashCode(), "0", message));
            }
        }
