...
namespace EFramework.Utility.Pinyin
{
    public static class Pinyin
    {
        ...
        /// <summary>
        /// 获取中文文本的拼音（驼峰命名法），如“中文拼音”-> "ZhongWenPinYin"
        /// </summary>
        /// <param name="text">编码为UTF8的文本</param>
        /// <returns>返回中文文本的拼音（驼峰命名法）</returns>
        public static string GetCamelCasePinyin(string text)
        {
            text = text.Trim();
            ...
            for (var i = 0; i < text.Length; ++i)
            {
                ...
                {
                    sb.Append(char.ToUpper(py[0]));
                    if (py.Length > 1)
                        sb.Append(py.Substring(1).ToLower());
                }
            }
            return sb.ToString();
        }
